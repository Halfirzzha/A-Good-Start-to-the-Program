                    ...
                    Tab::make(__('ui.system_settings.tabs.ai'))
                        ...
                        ->schema([
                            ...
                            Section::make('Multi-Provider AI (Enterprise)')
                                ...
                                ->persistCollapsed()
                                ->schema([
                                    // API Keys Grid - Tier 1 (Fastest & Cost-Effective)
                                    Grid::make(2)
                                        ->schema([
                                            TextInput::make('groq_api_key')
                                                ->label('Groq API Key (Priority 1 - Ultra Fast)')
                                                ->prefixIcon('heroicon-o-bolt')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Free tier! Llama 3.3, Mixtral → console.groq.com')
                                                ->maxLength(191),
                                            TextInput::make('deepseek_api_key')
                                                ->label('DeepSeek API Key (Priority 2 - Best Value)')
                                                ->prefixIcon('heroicon-o-academic-cap')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('DeepSeek-V3, R1 reasoning → platform.deepseek.com')
                                                ->maxLength(191),
                                            TextInput::make('xai_grok_api_key')
                                                ->label('xAI Grok API Key (Priority 3 - Real-time)')
                                                ->prefixIcon('heroicon-o-cpu-chip')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Grok 3, real-time knowledge → console.x.ai')
                                                ->maxLength(191),
                                            TextInput::make('openai_api_key')
                                                ->label('OpenAI API Key (Priority 4 - Industry Standard)')
                                                ->prefixIcon('heroicon-o-sparkles')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('GPT-4o, GPT-4o-mini → platform.openai.com')
                                                ->maxLength(191),
                                            TextInput::make('anthropic_api_key')
                                                ->label('Anthropic API Key (Priority 5 - Safety-First)')
                                                ->prefixIcon('heroicon-o-shield-check')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Claude 3.5 Sonnet, Opus → console.anthropic.com')
                                                ->maxLength(191),
                                            TextInput::make('gemini_api_key')
                                                ->label('Google Gemini API Key (Priority 6 - Multimodal)')
                                                ->prefixIcon('heroicon-o-globe-alt')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Free tier! Gemini 2.0 → aistudio.google.com')
                                                ->maxLength(191),
                                            TextInput::make('mistral_api_key')
                                                ->label('Mistral AI API Key (Priority 7 - European)')
                                                ->prefixIcon('heroicon-o-language')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Mistral Large, Codestral → console.mistral.ai')
                                                ->maxLength(191),
                                            TextInput::make('cohere_api_key')
                                                ->label('Cohere API Key (Priority 8 - RAG Specialist)')
                                                ->prefixIcon('heroicon-o-magnifying-glass')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('Command R+, embeddings → dashboard.cohere.com')
                                                ->maxLength(191),
                                            TextInput::make('openrouter_api_key')
                                                ->label('OpenRouter API Key (Priority 9 - Universal Gateway)')
                                                ->prefixIcon('heroicon-o-arrows-right-left')
                                                ->password()
                                                ->revealable()
                                                ->visible(fn (): bool => self::canEditSecrets() && self::canManageAISettings())
                                                ->disabled(fn (): bool => ! self::canEditSecrets() || ! self::canManageAISettings())
                                                ->helperText('100+ models, FREE options! → openrouter.ai')
                                                ->maxLength(191),
                                        ]),
